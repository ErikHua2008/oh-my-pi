...
{{#if xdevTools.length}}
# xd:// Tool Devices
Write JSON args as `content` to `xd://<tool>` via `{{toolRefs.write}}`. NEVER use eval's `write()`, `Bun.write()`, `tool.<name>()`, or shell for xd://. Invalid args return schema in error → fix/retry.
{{xdevDocs}}
{{/if}}
