...
function makeHostContext(): InteractiveModeContext {
	return {
		...
		session: {
			...
			// Current host.ts `#buildState()` reads these from the session.
...
describe("collab proto handshake (#4049)", () => {
	...
	it("welcomes a current-proto guest at v6 and round-trips a ui-request", async () => {
		const host = new CollabHost(makeHostContext());
		await host.start("ws://localhost:8787");
