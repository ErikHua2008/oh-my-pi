...
import type {
	DesktopNativeTranscriptFileLink,
...
const MAX_FILE_LINKS = 4;
const MAX_FILE_PATH = 32_768;
...
function isAbsoluteWindowsFilePath(path: string): boolean {
	if (/^[A-Za-z]:[\\/]/.test(path)) return true;
	if (!/^[\\/]{2}/.test(path) || /^[\\/]{2}\.[\\/]/.test(path)) return false;
	if (/^[\\/]{2}\?[\\/]/.test(path)) {
		return /^[\\/]{2}\?[\\/](?:[A-Za-z]:[\\/]|UNC[\\/][^\\/]+[\\/][^\\/]+)/i.test(path);
	}
	return /^[\\/]{2}[^\\/]+[\\/][^\\/]+/.test(path);
}

function normalizeNativeFileTarget(value: string): string | null {
	let path = value.trim();
	if (path.startsWith("<") && path.endsWith(">")) path = path.slice(1, -1).trim();
	// Codex file links may append :line or :line:column. These identify an
	// editor location and are not part of a valid Windows filename.
	path = path.replace(/:(\d+)(?::\d+)?$/, "");
	if (path.length === 0 || path.length > MAX_FILE_PATH || !isAbsoluteWindowsFilePath(path)) return null;
	return path;
}

function fileLinkKey(path: string): string {
	return path.replaceAll("/", "\\").toLocaleLowerCase();
}

/** Extract only explicit local-file references from an assistant answer. */
export function nativeAssistantFileLinks(text: string): DesktopNativeTranscriptFileLink[] {
	const result: DesktopNativeTranscriptFileLink[] = [];
	const seen = new Set<string>();
	const append = (target: string, label: string): void => {
		const path = normalizeNativeFileTarget(target);
		if (path === null) return;
		const key = fileLinkKey(path);
		...
		const fallback =
			path
				...
				.split(/[\\/]/)
				.pop() || "本机文件";
		const cleanLabel = label.trim().replaceAll(/\s+/g, " ");
		result.push({ path, label: (cleanLabel || fallback).slice(0, 256) });
	};

	// Markdown links are the normal report hand-off generated by Codex/OMP.
	const markdownLink = /!?\[([^\]\r\n]{0,512})\]\(\s*(<[^>\r\n]+>|(?:[A-Za-z]:[\\/]|[\\/]{2})[^)\r\n]+)\s*\)/g;
	for (const match of text.matchAll(markdownLink)) {
		append(match[2] ?? "", match[1] ?? "");
		if (result.length >= MAX_FILE_LINKS) return result;
	}
	// An absolute path deliberately set off as inline code is explicit enough
	// to expose too, without turning arbitrary command output into file cards.
	const inlinePath = /`((?:[A-Za-z]:[\\/]|[\\/]{2})[^`\r\n]+)`/g;
	for (const match of text.matchAll(inlinePath)) {
		append(match[1] ?? "", "");
		if (result.length >= MAX_FILE_LINKS) break;
	}
	return result;
}
	...
	return {
		...
		estimatedHeight:
			(kind === "reasoning" || kind === "tool") && (flags & NativeRowFlag.Expandable) !== 0
				? 42
				: estimatedHeight(bounded, kind) + mediaIds.length * 176 + fileLinks.length * 72,
		mediaIds,
		durationMs,
		processItems,
		fileLinks,
...
function nativeTurnRows(turn: TranscriptAssistantTurn): DesktopNativeTranscriptRow[] {
	...
	if (turn.finalEntry !== undefined) {
		...
		const responseText = assistantResponseText(message) || "…";
		rows.push(
			row(
				turn.finalEntry.id,
				...
				failed ? [] : nativeAssistantFileLinks(responseText),
			),
		);
	}
	return rows;
...
export function projectNativeStream(
	...
	if (stream !== null) {
		...
		if (assistantStreamIsFinal(stream)) {
			...
			const responseText = assistantResponseText(stream) || "…";
			return row(
				id,
				failed ? "error" : "assistant",
				responseText,
