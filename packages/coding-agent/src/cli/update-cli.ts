...
async function fetchLatestManifest(
	...
	if (!response.ok) {
		throw new Error(t("Failed to fetch release info for {pkg}: {reason}", { pkg, reason: response.statusText }));
	}
...
async function updateViaBun(release: ReleaseInfo): Promise<void> {
	...
	try {
		...
		if (pruneResult && pruneResult.removedEntries > 0) {
			console.log(chalk.dim(t("Pruned {count} stale Bun cache entries", { count: pruneResult.removedEntries })));
		}
	} catch (err) {
